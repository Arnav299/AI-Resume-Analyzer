...
const filePath = path.join(__dirname, 'src', 'pages', 'OrgLoginPage.jsx');
...
// Fix the button that got changed if any (wait, btn is text-white, let's make sure it's intact)
// My replace above was specific to text-3xl font-extrabold text-white, the button is "w-full text-base py-3 text-white shadow-lg"
// So button text-white is untouched.

fs.writeFileSync(filePath, content, 'utf8');
console.log('Replacements complete for OrgLoginPage.jsx');
